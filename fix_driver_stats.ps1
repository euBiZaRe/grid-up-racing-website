...
function Build-RatingGroup([string]$catName, [string]$cls, [string]$sr, [string]$ir, [string]$w) {
    return @"
                <div class="rating-group">
                    <div class="rating-label">
                        <span>$catName <span class="license-badge $cls">$sr</span></span>
                        <span>$ir</span>
                    </div>
                    <div class="rating-bar-bg"><div class="rating-bar-fill" style="width: $w%;"></div></div>
                </div>
"@
}
...
function Update-DriverFile {
    ...
    $scHtml = Build-RatingGroup 'Sports Car' (Get-LicenseClass $scSR) $scSR $sc (Get-BarWidth $sc)
    $fmHtml = Build-RatingGroup 'Formula'    (Get-LicenseClass $fmSR) $fmSR $fm (Get-BarWidth $fm)
    $ovHtml = Build-RatingGroup 'Oval'       (Get-LicenseClass $ovSR) $ovSR $ov (Get-BarWidth $ov)
    $dtHtml = Build-RatingGroup 'Dirt'       (Get-LicenseClass $dtSR) $dtSR $dt (Get-BarWidth $dt)

    $replacement = "$scHtml`n`n$fmHtml`n`n$ovHtml`n`n$dtHtml"
    
    $opts = [System.Text.RegularExpressions.RegexOptions]::Singleline
    $pattern = '(<h3 class="section-title"[^>]*>iRacing Ratings</h3>).*?(?=</div>\s*<!-- Career Stats Section)'
    
    $repl = "`$1`n                `n$replacement`n            "
    
    $newContent = [System.Text.RegularExpressions.Regex]::Replace($content, $pattern, $repl, $opts)
