...
async function updateAuthUI(user, isTentative = false) {
    ...
    if (user) {
        ...
        if (navActions) {
            ...
            const portalLink = `<a href="${basePath}portal.html" id="navbar-portal-btn" style="position: relative; ${btnBase} background: linear-gradient(90deg, #00D2FF, #0066B1); color: #000000; border: none; box-shadow: 0 0 12px rgba(0,210,255,0.4);">Portal<span id="portal-badge" style="display: ${PORTAL_HAS_UNREAD ? 'block' : 'none'}; position: absolute; top: -4px; right: -4px; width: 8px; height: 8px; background-color: #ff3b30; border-radius: 50%; border: 1px solid #fff; box-shadow: 0 0 4px rgba(255,59,48,0.6);"></span></a>`;
